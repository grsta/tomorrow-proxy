...
app.get("/weather", async (req, res) => {
  try {
    ...
    const currentWeather = {
      ...
      conditionText: weatherCodes[currentCode] || "Unknown",
      icon: weatherIcons[currentCode] || weatherIcons[0],
      overlay: overlayVideos[String(currentCode)] || overlayVideos["0"],
      temp_f: data.current_weather?.temperature != null
        ? (data.current_weather.temperature * 9/5 + 32).toFixed(1)
        : null,
      feelslike_f: data.hourly?.apparent_temperature?.[0] != null
        ? (data.hourly.apparent_temperature[0] * 9/5 + 32).toFixed(1)
        : null,
      windspeed_mph: data.current_weather?.windspeed != null
        ? (data.current_weather.windspeed * 0.621371).toFixed(1)
        : null,
      winddirection: degreesToDirection(data.current_weather?.winddirection),
